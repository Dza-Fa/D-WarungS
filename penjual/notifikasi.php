...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['mark_all_read'])) {
    $query = "UPDATE notifications SET is_read = 1 WHERE user_id = ? AND is_read = 0 AND role = 'pedagang'";
...
$where = "user_id = ? AND role = 'pedagang'";
...
// Get unread count
$unread_count = getRow("SELECT COUNT(*) as count FROM notifications WHERE user_id = ? AND is_read = 0 AND role = 'pedagang'", [$_SESSION['user_id']])['count'] ?? 0;
?>
...
<div class="page-header">
    ...
    <p class="page-subtitle">Pesanan baru, update pesanan, dan review dari pembeli</p>
</div>
...
<div style="margin-bottom: 1.5rem; display: flex; gap: 0.5rem;">
    <a href="notifikasi.php" class="btn <?php echo $filter === 'all' ? 'btn-primary' : 'btn-outline-secondary'; ?> btn-sm">
        📊 Semua (<?php echo count(getRows("SELECT id FROM notifications WHERE user_id = ? AND role = 'pedagang'", [$_SESSION['user_id']])); ?>)
    </a>
        ...
        <div class="card-body">
            <div class="empty-state" style="padding: 3rem;">
                ...
                <div class="empty-state-subtext" style="font-size: 0.9rem; color: #999;">
                    Notifikasi pesanan baru dan review akan muncul di sini
                </div>
            </div>
        </div>
    </div>
                    ...
                    <div style="display: flex; gap: 0.75rem; align-items: start;">
                        ...
                        <div style="flex: 1;">
                            ...
                            <?php if (!empty($notif['order_id'])): ?>
                                <a href="pesanan.php?order_id=<?php echo $notif['order_id']; ?>" class="btn btn-sm btn-outline-primary" style="margin-top: 0.5rem;">
                                    👁️ Lihat Pesanan
                                </a>
                            <?php elseif (strpos($notif['type'], 'review') !== false): ?>
                                <a href="rating_menu.php" class="btn btn-sm btn-outline-primary" style="margin-top: 0.5rem;">
                                    ⭐ Lihat Review
                                </a>
                            <?php endif; ?>
                        </div>
